...
# 2. showAddDialog visual auto-save (lines 140/143 roughly)
content = content.replace(
...
content = content.replace(
    '''            OpinionEditDialog(
                ...
                onConfirm = { opinion, confidence, referenceLinks, imagePath ->
                    onAddOpinion(opinion, confidence, referenceLinks, imagePath)
                    showAddDialog = false
                }
            )''',
    '''            OpinionEditDialog(
                ...
                onNavigateToImageEditor = onNavigateToImageEditor,
                onConfirm = { opinion, confidence, referenceLinks, tags, imagePath ->
                    onAddOpinion(opinion, confidence, referenceLinks, tags, imagePath)
                    showAddDialog = false
                }
            )'''
)
...
content = content.replace(
    '''            val initialTags by (if (opinionToEdit != null) onLoadTags(opinionToEdit.id) else kotlinx.coroutines.flow.flowOf(emptyList())).collectAsState(initial = emptyList())
            OpinionEditDialog(
                ...
                initialConfidence = opinionToEdit.confidenceScore,
                ...
                onNavigateToImageEditor = onNavigateToImageEditor,
                onConfirm = { text, confidence, referenceLinks, selectedTags, imagePath ->
                    if (opinionToEdit != null) {
                        onUpdateOpinion(opinionToEdit.id, text, confidence, opinionToEdit.createdAt, referenceLinks, selectedTags, imagePath)
                    }
                    showEditDialogId = null
                }
            )''',
...
# If the second replace didn't work because initialTags was missing in original
content = content.replace(
    '''            OpinionEditDialog(
                viewModel = referenceSelectorViewModel,
                templates = templates,
...
# Find the start of Knowledge Connections row:
knowledge_conn_start = '''                    // Action row: Knowledge Connections + Add Image button
                    Row(
